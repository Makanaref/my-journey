...
old_block = '''                <div class="hero-card-title hero-card-title-teal">Mint Token</div>
                ...
                <div class="hero-card-arrow hero-card-arrow-teal">'''

# Find the index and insert after the full line containing the arrow div and closing </a>
idx = content.find(old_block)
if idx == -1:
    print("ERROR: anchor still not found!")
else:
    # find the end of the closing </a> tag right after this block
    close_a_idx = content.find("</a>", idx)
    if close_a_idx == -1:
        print("ERROR: closing </a> not found!")
    else:
        ...
        if old_css not in content:
            print("SUCCESS (card added): " + str(True) + " | WARNING: css anchor not found, styles not added.")
        else:
            content = content.replace(old_css, new_css, 1)
            print("SUCCESS: Mint NFT card and styles added to homepage.")
